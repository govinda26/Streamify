...
function MyChannelPage() {
  ...
  const loggedInUserId = useMemo(() => localStorage.getItem("user_id"), []);
  ...
  const isOwnChannel = useMemo(() => {
    ...
    if (loggedInUserId && channel?._id) {
      return channel._id.toString() === loggedInUserId;
    }
    if (loggedInUsername && channel?.username) {
      return (
        loggedInUsername.toLowerCase() === channel.username.toLowerCase()
      );
    }
    return false;
  }, [channel, loggedInUserId, loggedInUsername]);
              ...
              <div className="flex flex-wrap gap-4 pb-4 pt-6">
                ...
                <div className="mr-auto inline-block">
                  <h1 className="font-bold text-xl">
                    {channelLoading ? "Loading…" : channel?.fullName || "Channel"}
                  </h1>
                ...
                {tabs.map((tab) => (
                  <li key={tab} className="w-full">
                    <button
                      ...
                      className={`w-full cursor-pointer border-b-2 px-3 py-1.5 transition-colors ${
                        activeTab === tab
                  ...
                  <div className="w-full max-w-sm text-center">
                    ...
                    {isOwnChannel ? (
                      <button
                        ...
                        className="mt-4 inline-flex items-center gap-x-2 bg-[#ae7aff] px-3 py-2 font-semibold text-black transition-all duration-150 hover:scale-[1.03] hover:bg-[#9f86ff]"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
                  ...
                  <div className="grid grid-cols-[repeat(auto-fit,_minmax(300px,_1fr))] gap-4 pt-2">
                    {videos.map((video) => (
                      <Link
                        to={`/video/${video._id}`}
                        key={video._id}
                        className="group block overflow-hidden rounded-lg border border-white/10 bg-[#141414] hover:border-[#ae7aff]"
                      >
                        <div className="relative w-full pt-[56%]">
                          <img
                            ...
                            loading="lazy"
                          />
                          <span className="absolute bottom-2 right-2 rounded bg-black/70 px-2 py-1 text-xs font-semibold">
